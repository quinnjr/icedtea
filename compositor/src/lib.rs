...
/// Boot and run the compositor: loads config, wires the D-Bus service, the
/// wallpaper decode worker, the config-reload worker, and the chosen
/// backend (nested winit or DRM), then runs the event loop until
/// `apply_action("quit")` (or an equivalent D-Bus `Quit` command) stops it.
pub fn run() {
