    ...
    pub fn handle_command(&mut self, cmd: crate::dbus::DbCommand) -> Option<()> {
        ...
        match cmd {
            ...
            DbCommand::GetState(reply_tx) => {
                ...
                // M8: the model snapshot carries no input state, so fill the
                // keyboard indicator fields here from the live runtime. No
                // runtime (every model-only unit test) reads as no layout,
                // uninhibited, matching the defaults.
