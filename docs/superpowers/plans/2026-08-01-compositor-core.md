...
- No unsafe in icedtea code except (a) `unsafe { redb::Database::create }` / `Database::open` (required by redb's mmap API) and (b) the calloop `Generic::get_mut` block registering the Wayland `Display` as an event source (no safe API exists in calloop 0.14/smithay 0.7; smallvil and anvil use the identical pattern) — each with an accurate safety comment stating the real obligation (the display is never dropped while the source is registered).
