...
- No unsafe in icedtea code except (a) `unsafe { redb::Database::create }` / `Database::open` (required by redb's mmap API) (b) the calloop `Generic::get_mut` block registering the Wayland `Display` as an event source (no safe API exists in calloop 0.14/smithay 0.7; smallvil and anvil use the identical pattern), and (c) `std::env::set_var` for `WAYLAND_DISPLAY` during single-threaded startup (`unsafe fn` under edition 2024; no safe alternative) — each with an accurate safety comment stating the real obligation.
