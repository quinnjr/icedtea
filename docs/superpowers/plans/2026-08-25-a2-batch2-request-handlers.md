...
**Architecture:** Unlike Batch 1's passive globals, each of these needs the `wlr` crate to expose an event (a wlroots signal listener fanned out to a new handler-trait method) AND the compositor to act on it. cursor-shape reuses the crate's existing `wlr_cursor` + `wlr_xcursor_manager` (created with the seat) to set a named xcursor; xdg-activation runs a focus-steal policy and, when it declines to steal focus, raises an additive `attention` bit on the `org.icedtea.Compositor` window state; gamma-control is scene-integrated — `Runtime::create_gamma_control_manager` hands the manager to `wlr_scene_set_gamma_control_manager_v1`, and the scene itself fits and applies each ramp on its own output commits (and sends `failed` on a rejected one), so the compositor's `gamma_control_changed` handler is NOTIFICATION-ONLY: no LUT stash, no `OutputHandler::frame` commit-path change (see Task 9's ruling — wlroots 0.20 has no `wlr_output_state_set_gamma_lut`).
