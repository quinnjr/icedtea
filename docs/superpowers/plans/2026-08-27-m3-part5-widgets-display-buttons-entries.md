...
- **D12 — P5 does edit `view/controller.rs`, `view/reconcile.rs` and `view/app.rs`, contrary to D8's "no edit" wording and the File Structure row below.** Added 2026-08-31 in the whole-part fix wave. `Controller<Msg>` gains `std::any::Any` as a supertrait so `widgets::child_slot` can downcast; `view::build_controller` delegates to `widgets::build_controller` and `view::generic_controller` is split out of it; `reconcile.rs` routes application children through `child_slot(..).unwrap_or_else(root)`; four tests move from `Kind::Button` to `Kind::MenuButton` or flip a class-order assertion. Recorded in the contract as **§10 P5-D32**, with **§11 E4** amended.
- **D13 — P5 fixes `route`'s `aim` in `view/app.rs` to resolve to the innermost `Instance` in the hit chain.** Added 2026-08-31 in the whole-part fix wave. Without it every widget whose chrome lives on a controller-owned subnode (`DropDown`, `MenuButton`, `ColorDialogButton`) never receives its own click. `opening_a_drop_down_and_picking_an_item_updates_the_button` and `clicking_a_colour_button_opens_its_dialog_and_repaints_the_swatch` are un-ignored. Recorded in the contract as **§10 P5-D33**.
...
| `ui/src/view/app.rs` | **Modify** (D13, was "Untouched"). No stylesheet setter — P4's `App::with_sheet` (P4 D11) covers D8 — but `route`'s `aim` is fixed here. |
| `ui/src/view/controller.rs` | **Modify** (D12). `Any` supertrait; `build_controller` delegates; `generic_controller` split out. |
| `ui/src/view/reconcile.rs` | **Modify** (D12). Application children attach under `widgets::child_slot`. |
