...
**Interfaces:**
- Produces: `session::logind::SleepInhibitor` (Drop guard that closes the fd; `#[must_use]`); `PrepareForSleep(bool)` handling — on `true`, decide via `policy::decide`, spawn the locker via a shell-interpreted `sh -c <locker_command>` in its own process group (deliberately *not* the compositor's hardened no-shell `spawn` arm, which parses an argv; the shell wrapper is what makes the configured string carry its own arguments), poll `WmClient::is_locked()` every 100ms up to 3s, then release; on `false` re-acquire. `Manager.Inhibit("handle-power-key:handle-suspend-key:handle-hibernate-key", "icedtea-session", …, "block")` held for daemon lifetime. `Lock`/`Unlock` signal funnel: spawn/wait locker, kill on external unlock, `Session.Unlock()` when the locker exits.
...
**Interfaces:**
- Produces: `icedtea-session.service` (mirror `clipboard/systemd/icedtea-clipboard.service`); default bindings `XF86_PowerOff` → `spawn:icedtea-session lock`, `XF86_Sleep` → `spawn:icedtea-session suspend`, `XF86_Hibernate` → `spawn:icedtea-session hibernate` (per the spec's Milestone 3 note).
...
- **Known gaps:** the production `ZbusWmClient` is now exercised end-to-end against the *real* harness compositor: `session/tests/wm_real_compositor.rs` serves the production `org.icedtea.Compositor` interface (`icedtea_compositor::dbus::spawn_service`) over a private `dbus-daemon`, wired to the harness's own event stream and command queue, and drives `IsLocked()`/`lock_state()` both polarities, `subscribe_lock_changed` receiving the real `SessionLockChanged` signal emitted when a real `SessionLockClient` takes and releases the `ext_session_lock_manager_v1` lock, and `Quit()` — all through `ZbusWmClient::new()` (skips `LEXSKIP:` without `dbus-daemon`/headless wlroots). `terminate_locker`'s group-kill failure branch is likewise now covered hermetically by `flow::tests::a_failed_group_kill_falls_back_to_kill_and_reap_without_a_clean_unlock`, via the `#[cfg(test)]` `group_kill` seam mirroring `locker_shell`. What **remains** uncovered: a full session-daemon boot (config + `ext_idle_notifier_v1` + logind over the *system* bus) against a real compositor with a configured locker — `session/tests/idle_lock.rs` already boots the idle→lock flow against the real harness compositor with a configured stub locker, but through the `RecordingWm` seam rather than `ZbusWmClient`, and no test wires the daemon's own boot (config/logind) to a real compositor; that half needs a system bus and is out of scope here. What **is** still covered on a live bus: criterion 5's `IsLocked()` method and `SessionLockChanged` signal round-trip over a real session bus (`compositor/src/dbus.rs::is_locked_and_session_lock_changed_round_trip_over_a_session_bus`), criterion 6's CLI route reaching a real `org.icedtea.Session` service over a private bus (`session/tests/cli.rs`), and logind session discovery/inhibit/power/lock/unlock/signal handling against a fake logind over a private bus (`session/tests/fake_logind.rs`).
