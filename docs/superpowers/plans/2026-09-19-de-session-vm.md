...
Add `Halign`/`Valign`/`Hexpand`/`Vexpand` to `CENTRAL_UNIVERSAL` (`ui/src/widgets/state.rs`) and add arms to `Universal::apply` (`ui/src/widgets/mod.rs`) that start from `child_layout_or_centred(node)` — the recorded layout when one exists, else the centred *unaligned* base (`Align::Center` on both axes), because `Align::default() == Fill` and `child_layout_of`'s `unwrap_or_default()` would otherwise collapse the unaligned state into a stretch — mutate only the named axis, and `set_child_layout(node, cl)`; never a fresh `ChildLayout::default()`, which would clobber the `absolute`/`grid`/`margin`/other-axis fields that `overlay`, `grid`, `paned`, `frame` and `stack` already set. A non-bool expansion value restores `false`; a non-align value restores the centred base, so setting one axis never disturbs the other.
