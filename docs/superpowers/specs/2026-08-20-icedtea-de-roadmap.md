...
| **Session & system plumbing** | **logind/seat** (`icedtea-session`: suspend/hibernate/poweroff/reboot/lock, lock-on-sleep, idle→lock, sleep + power-key inhibitors; lid-close via logind's own defaults) | **notification daemon** (`org.freedesktop.Notifications`); **xdg-desktop-portal + PipeWire** (Flatpak file-chooser, screencast/screen-share); **polkit agent**; **secret service/keyring**; XDG autostart |
