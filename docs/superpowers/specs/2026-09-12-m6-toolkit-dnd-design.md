...
1. At the threshold crossing the runtime offers the drag to
   `dnd::DragSeat` — `offer_drag(&SeatDragRequest{serial, mimes, payload})`
   — with the recorded arming press serial, the offered MIME list, and the
   bytes. On `Accepted` the compositor owns target routing and the toolkit
   stops its own in-surface hit-testing; `Refused`/no seat keeps the drag
   internal unchanged.
...
- **Destination-in routing.** A drag delivered *into* this client's surface
  is not routed to toolkit targets yet: `wl_data_device.enter`/`motion`/
  `drop` on the destination side are ignored by `WindowState`. The source
  half works; same-surface drops keep using §§2–5 because the toolkit only
  offloads once the seat accepts and, today, it remains the source's own
  surface that drives the bytes.
...
- **Compositor-reported outcome.** When a drag is offloaded, the toolkit's
  own `DragEnd{dropped}` reports the in-surface target state (usually
  `false`); the authoritative drop is the compositor's (`wl_data_source`
  `dnd_drop_performed`/`dnd_finished`), which `ClientSeat` uses only to
  retire the source. Folding that back into a model message is the follow-up.

The seam: `dnd::DragSeat` — `offer_drag(&SeatDragRequest{serial, mimes,
payload}) -> SeatDragResult` — with `StubSeat` (scripted accept/refuse + call
log) for the toolkit half and `ClientSeat` for a live window. Unit tests
prove the request shape (serial = the arming press serial; MIME list = the
offered list; bytes = the payload) against a recording stub, and that an
accepting stub suppresses internal target routing.
...
- `ui/tests/dnd.rs` (NEW, offscreen `App` + `ScriptStep` pointer events,
  ...
  6. a recording `DragSeat`: a `Refused` seat leaves the drag internal and
     ...
     an `Accepted` seat offloads the drag and the toolkit stops routing
     `DragEnter`/`DragMotion`/`Drop` to the in-surface target.
